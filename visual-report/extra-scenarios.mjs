...
export const extraVisualReportScenarios = [
  ...
  scene("preferences-color-restore", "我的偏好 · 恢复云盘颜色", "取消 E 层个人颜色后刷新，接口夹具返回持久化结果。", `${driveRoute}/preferences?view=colors`, selector(".preference-row:first-child input[type=color]"), [click("button", "E · Ensemble 恢复默认颜色"), { type: "waitText", text: "已保存" }, { type: "reload" }]),
  ...
  reader("reader-layer-save-failure", "阅读器 · 显示设置保存失败", "故意注入 503，保留原值并显示恢复动作。", selector(".reader-layer-feedback"), [showControls, click("button", "图层"), click("checkbox", "显示 E · Ensemble")]),
  ...
  scene("preferences-display-save", "我的偏好 · 保存默认显示", "关闭 E 默认显示后刷新，保留用户设置。", `${driveRoute}/preferences`, selector('.preference-display-toggle input[aria-label="E · Ensemble 默认显示"]:not(:checked)'), [click("checkbox", "E · Ensemble 默认显示"), { type: "waitText", text: "已保存" }, { type: "reload" }]),
