...
export const extraVisualReportScenarios = [
  ...
  scene("preferences-color-restore", "我的偏好 · 恢复云盘颜色", "取消 E 层个人颜色后刷新，接口夹具返回持久化结果。", `${driveRoute}/preferences?view=colors`, selector(".preference-row:first-child input[type=color]"), [click("button", "E · 全体 恢复默认颜色"), { type: "waitText", text: "已保存" }, { type: "reload" }]),
  ...
  reader("reader-layer-save-failure", "阅读器 · 显示设置保存失败", "故意注入 503，保留原值并显示恢复动作。", selector(".reader-layer-feedback"), [showControls, click("button", "看哪些批注"), click("checkbox", "显示 E · 全体")]),
  ...
  scene("preferences-display-save", "我的偏好 · 保存默认显示", "关闭 E 默认显示后刷新，保留用户设置。", `${driveRoute}/preferences`, selector('.preference-display-toggle input[aria-label="E · 全体 默认显示"]:not(:checked)'), [click("checkbox", "E · 全体 默认显示"), { type: "waitText", text: "已保存" }, { type: "reload" }]),
  ...
  reader("reader-layers-mobile-personal", "阅读器 · 手机个人层", "手机面板滚动到底，显示个人层与隐私说明。", selector(".layer-section--personal .layer-card"), [showControls, click("button", "看哪些批注"), { type: "scrollIntoView", selector: ".layer-section--personal .layer-card" }]),
