...
export const extraVisualReportScenarios = [
  ...
  scene("preferences-color-restore", "我的偏好 · 恢复云盘颜色", "取消 E 层个人颜色后刷新，接口夹具返回持久化结果。", `${driveRoute}/preferences?view=colors`, selector(".preference-row:first-child input[type=color]"), [click("button", "Ensemble 恢复默认颜色"), { type: "waitPreferences" }, { type: "reload" }]),
  ...
  reader("reader-layer-save-failure", "阅读器 · 显示设置保存失败", "故意注入 503，保留本机选择并显示重试动作。", selector(".reader-layer-feedback"), [showControls, click("button", "笔记图层"), click("checkbox", "显示 Ensemble")]),
  ...
  reader("reader-offline-failure", "阅读器 · 离线下载失败", "离线元数据下载注入 503，提供重试并按已校验副本反馈可用性。", role("button", "重试保存离线副本"), [showControls, click("button", "更多"), { type: "armFailures" }, click("button", "保存供离线使用")]),
  scene("preferences-display-save", "我的偏好 · 保存默认显示", "关闭 E 默认显示后刷新，保留用户设置。", `${driveRoute}/preferences`, selector('.preference-display-toggle input[aria-label="Ensemble 默认显示"]:not(:checked)'), [click("checkbox", "Ensemble 默认显示"), { type: "waitPreferences" }, { type: "reload" }]),
  ...
  reader("reader-layers-mobile-personal", "阅读器 · 手机个人层", "手机面板滚动到底，显示个人层与隐私说明。", selector(".layer-section--personal .layer-card"), [showControls, click("button", "笔记图层"), { type: "scrollIntoView", selector: ".layer-section--personal .layer-card" }]),
  reader("reader-dense-landscape-page", "阅读器 · 混合纸型第三页", "八页定位跳到横向纸张，重新适合整页并保持中心定位。", selector('.page-reader__sheet[data-page-turn-current][data-page-number="3"] [data-pdf-canvas-active]'), [showControls, { type: "pressRole", role: "slider", name: "跳转页码", key: "Home" }, { type: "pressRole", role: "slider", name: "跳转页码", key: "ArrowRight" }, { type: "pressRole", role: "slider", name: "跳转页码", key: "ArrowRight" }], { device: "landscape", dense: true, expectedAnnotation: false }),
