...
诊断报告增加可选 `step` 和 `errorType` 固定枚举，服务端严格验证；保留原有报告格式、匿名提交和 30 天期限。阅读器同步记录图层准备、推送、拉取检查点/请求、应用云端结果、本机草稿与冲突操作的失败。本机副本检查区分PDF 文件、笔记快照验证以及本机读取异常；异常保持原样交还业务流程，白名单之外的名称降为 OtherError。
