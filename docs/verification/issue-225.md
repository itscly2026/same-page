...
阅读偏好先反馈到谱面，再通过 IndexedDB 事务保存；界面区分正在保存、本机失败、待同步和已同步。同项最后意图在 Web Locks 内串行发送，其他项独立推进；重开与网络恢复可重新发送。没有 Web Locks 时保留本机待同步并提示，不能声称已同步。偏好投影只改变已有层的显示字段，不增加层或授予权限。账户切换、退出和已确认退盘阻止旧会话继续回写。
