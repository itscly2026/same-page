...
CI 的严格视口回归另发现：估算页高换成真实页高后也没有重新定位。失败几何显示选中第 8 页、scrollTop 8175，但第 8 页实际从视口 y=491 开始，第 7 页仍在视口上部。恢复标记同时记录几何是否已就绪，在真实页高到达时校正一次，不因缩放重复吸附。短末页还会受浏览器最大滚动位置限制而临时选中前一页；必须保留原始恢复目标直到真实几何定位完成，不能拿临时可见页覆盖目标。严格回归同时等待目标页可见、当前页与视口一致，原先末页丢失可在本地复现。
