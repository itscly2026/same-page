...
在支持 pen hover 的设备上，悬停不显示工具文字：画笔以约 2.5 CSS px 的淡小点提示落点，整条橡皮为淡阴影；荧光笔使用与书写相同的笔头方向，预览透明度上限为 30%。没有悬停输入的设备仍可使用所有基础编辑功能。
...
- 同一笔回划或交叉合并为均匀的透明涂层，采样密度增加或原地停顿不会额外叠色；抬笔后再次覆盖才加深。两笔 30% 涂层交叉为约 51% 合成不透明度。连续笔迹仍是一个对象，一次撤销即可移除。
- 新建荧光笔默认扁头，可切回圆头。历史圆头对象保持圆头，选中后可修改笔头。扁头方向只随 `twist` 绕笔轴旋转，`tiltX`/`tiltY` 不改变方向或长度；浏览器未提供旋转时使用固定斜角。记录的姿态参与重绘与导出。
