...
const guides: Record<InstallGuide, { title: string; steps: string[] }> = {
  ...
  "android-external": { title: "Android · App 内打开", steps: ["打开当前 App 的菜单，选择“在浏览器中打开”。", "也可以复制下方网址，用 Chrome 或 Edge 打开；其他浏览器也可以尝试。", "在浏览器中再次点击“安装合谱”，先查看权限说明，再继续添加。"] },
...
export function InstallProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const ready = (event: Event) => { event.preventDefault(); if (!wechat) setPrompt(event as InstallPromptEvent); };
  ...
  return <InstallContext value={{ hidden: installed, suggest: !wechat && !installed && !dismissed && readInstallPreference("install-seen-score"), open: show, dismiss }}>
    {children}
    <ModalOverlay className="modal-overlay" isOpen={open && !installed} onOpenChange={setOpen} isDismissable>
      <Modal className="app-modal install-modal"><Dialog className="app-dialog install-dialog">
        ...
        {android && <section className="install-advice" aria-label="Android 安装前说明"><strong>桌面快捷方式权限</strong><p>如系统询问，请允许当前浏览器“创建桌面快捷方式”；没有此项可跳过。</p><p className="install-network-note">Chrome 的<strong>安装过程</strong>可能需要 Google Play 服务，中国境内网络可能导致失败；日常打开合谱无需经过 Play Store。</p></section>}
        {prompt && <Button className="primary-button install-native" isDisabled={busy} onPress={() => void install()}><Download size={18} />已了解，打开安装提示</Button>}
