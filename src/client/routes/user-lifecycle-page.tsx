...
export default function UserLifecyclePage() {
  ...
  const deleting = params.get("view") === "delete";
  ...
  return <div className="app-page"><AppHeader actions={<BackButton className="header-action" to={choirId ? `/choirs/${choirId}` : deleting ? "/user" : "/drives"}>{choirId ? "返回云盘" : deleting ? "返回个人设置" : "返回所有云盘"}</BackButton>} /><main className="page-shell settings-page settings-ux lifecycle-page">
    <header className="settings-heading"><h1>{choirId ? "退出此云盘" : deleting ? "删除用户" : "个人设置"}</h1></header>
    {state ? state.deletion ? <section>
      ...
      {choirId && <section><h2>此云盘成员关系</h2>
        {state.memberships.filter(member => member.choirId === choirId).map((member) => <article className="lifecycle-member" key={member.id}>
          ...
          {member.status === "active" ? <Button className="secondary-button" isDisabled={busy || member.isOwner === 1} onPress={() => setConfirmation({ title: "退出此云盘", action: "确认退出", message: "退出后立即停止该成员关系的云端访问与同步；三十天内请有成员恢复权限的人恢复。已下载内容及本机未同步草稿会保留。公开体验的本人个人层仍可使用。", onConfirm: () => perform(`/api/choirs/${member.choirId}/memberships/${member.id}`, { action: "remove", expectedRevision: member.revision }, async () => { await navigate("/drives"); }) })}>退出云盘</Button> : <p>三十天内可联系云盘拥有者恢复；个人层不向拥有者开放。</p>}
        </article>)}
      </section>}
      {!choirId && !deleting && <section className="personal-settings-links">
        ...
        <Link className="settings-secondary-link" to="/diagnostics">帮助与诊断<span aria-hidden="true">›</span></Link>
        <Link className="settings-secondary-link" to="/about">关于与更新<span aria-hidden="true">›</span></Link>
        <Link className="settings-secondary-link settings-danger-link" to="?view=delete">删除用户<span aria-hidden="true">›</span></Link>
      </section>}
      {!choirId && deleting && <section>
    ...
    {message && <Button className="secondary-button" isDisabled={busy} onPress={() => void reload()}>重新读取状态</Button>}
