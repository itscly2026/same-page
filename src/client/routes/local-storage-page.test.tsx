...
it("offers cancellation then clears this device's file and updates its space", async () => {
  ...
  fireEvent.click(await screen.findByRole("button", { name: "清理 排练" }));
  fireEvent.click(screen.getByRole("button", { name: "取消" }));
  expect(await listLocalFiles()).toHaveLength(1);
  fireEvent.click(screen.getByRole("button", { name: "清理 排练" }));
  ...
  await waitFor(() => expect(screen.queryByRole("button", { name: "清理 排练" })).not.toBeInTheDocument());
});
