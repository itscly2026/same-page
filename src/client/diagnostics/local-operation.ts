...
import { diagnosticErrorCode, diagnosticCauseType, diagnosticErrorType, diagnosticScope } from "./diagnostics";
  ...
  try { return await action(); }
    ...
    if (!options.signal?.aborted && !(error instanceof LocalWorkspaceOwnerChangedError) && diagnosticErrorType(error) !== "AbortError") {
      report({ operation: step.startsWith("offline-") ? "storage" : "sync", category: "internal",
        stage: step === "sync-push" ? "push" : "prepare", step, errorType: diagnosticErrorType(error), causeType: diagnosticCauseType(error), errorCode: diagnosticErrorCode(error) });
    }
    throw error;
  }
}
