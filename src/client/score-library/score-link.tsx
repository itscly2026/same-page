...
export function ScoreLink({ userId, choirId, scoreId, local, experience = false, children, onOpen }: {
  experience?: boolean; userId?: string; choirId: string; scoreId: string; local: boolean; children: ReactNode; onOpen: () => void;
}) {
  ...
  return local && !ready
    ...
    : <Link className="file-row__open" to={`/choirs/${choirId}/scores/${scoreId}${experience ? "?experience=1" : ""}`} onClick={onOpen}>{children}</Link>;
}
