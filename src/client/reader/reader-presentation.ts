...
type Target = { document: ScoreDocument | null; page: number; editing: boolean };
export type PresentationSnapshot = { document: ScoreDocument | null; status: "pending" | "visible" | "failed" };
...
export class ReaderPresentation {
  private presented: ScoreDocument | null = null;
  ...
  load(document: ScoreDocument | null) {
    if (this.disposed) return;
    this.publish(this.hasPresented(document) ? "visible" : "pending", document);
  }
  hasPresented(document: ScoreDocument | null) {
