...
export function StyleFields({ tool, value, onChange }: { tool: AnnotationTool; value: ToolStyle; onChange(value: ToolStyle): void }) {
  return <div className="annotation-style-fields">
    ...
    {tool === "highlighter" && <div className="annotation-pressure-options" role="group" aria-label="荧光笔笔头">{(["round", "chisel"] as const).map(nib => <button type="button" key={nib} aria-pressed={value.nib === nib} onClick={() => onChange({ ...value, nib })}>{nib === "round" ? "圆头" : "扁头"}</button>)}</div>}
