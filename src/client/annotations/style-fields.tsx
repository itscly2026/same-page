...
export function StyleFields({ tool, value, onChange }: { tool: AnnotationTool; value: ToolStyle; onChange(value: ToolStyle): void }) {
  return <div className="annotation-style-fields">
    ...
    {tool === "ink" && <div className="annotation-pressure-options" role="group" aria-label="笔迹模式">{(["uniform", "pressure"] as const).map(mode => <button type="button" key={mode} aria-pressed={value.pressureMode === mode} onClick={() => onChange({ ...value, pressureMode: mode })}>{mode === "uniform" ? "等宽" : "压感"}</button>)}</div>}
    <svg className="annotation-style-sample" viewBox="0 0 240 52" aria-label="样式预览">
      {tool === "text" ? <text x="12" y="36" fontSize={value.fontScale * 700} fill="currentColor">渐弱，留意指挥</text> : <path d="M16 30 Q60 8 108 28 T224 22" fill="none" stroke="currentColor" strokeWidth={value.strokeWidth * 700} opacity={tool === "highlighter" ? value.opacity : 1} strokeLinecap="round" />}
    </svg>
  </div>;
}
