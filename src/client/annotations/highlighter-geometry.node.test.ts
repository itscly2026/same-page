...
describe("highlighter deposition", () => {
  it.each(["round","chisel"] as const)("darkens retraced %s strokes without sample-rate darkening", nib => {
    ...
    for (const x of [.3,.4,.5,.6,.7]) {
      ...
      expect(depth({...dense,points:[...dense.points,{x:.1,y:.5}]},x,.503)).toBe(2);
    }
  ...
  it("retains crossings and gives the same geometry for live prefixes and restored points", () => {
    ...
    expect(depth({...base,points},.5,.5)).toBe(2);
    expect(highlighterPasses({...base,points},1)).toEqual(highlighterPasses({...base,points:structuredClone(points)},1));
  });
  it.each(["round", "chisel"] as const)("keeps diagonal %s sampling and repeated stationary points at one coat", nib => {
    const points = Array.from({ length: 101 }, (_, i) => ({ x: .1 + i * .008, y: .2 + i * .006 }));
    const ink = { ...base, nib, points: points.flatMap(point => [point, { ...point }, { ...point }]) };
    for (let i = 20; i < 80; i += 10) expect(depth(ink, points[i]!.x, points[i]!.y)).toBe(1);
    const retraced = { ...base, nib, points: [{ x: .1, y: .5 }, { x: .9, y: .5 }, { x: .1, y: .5 }, { x: .9, y: .5 }] };
    expect(depth(retraced, .5, .5)).toBe(3);
  });
